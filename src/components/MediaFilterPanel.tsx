...
export const MediaFilterPanel: React.FC<MediaFilterPanelProps> = ({
  ...
  return (
    <div className="space-y-6">
      ...
      <form onSubmit={handleApply} className="flex items-center gap-2 max-w-3xl">
        <div className="relative flex-1">
          <input
            ...
            className={`w-full pl-11 pr-4 rtl:pr-11 rtl:pl-4 py-3 text-sm sm:text-base rounded-xl border transition-all ${
              theme === 'dark'
                ? 'bg-slate-900 border-slate-700 text-white placeholder-slate-400 focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500'
                : 'bg-white border-slate-300 text-slate-900 placeholder-slate-400 focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 shadow-xs'
            }`}
          />
        ...
        <button
          type="submit"
          className="px-6 py-3 rounded-xl bg-indigo-600 hover:bg-indigo-500 text-white font-bold text-sm shadow-md shadow-indigo-600/30 transition-all cursor-pointer whitespace-nowrap flex items-center gap-2"
        >
      ...
      {/* Main Filter Panel */}
      <div
        className={`p-5 sm:p-6 rounded-2xl border transition-all space-y-6 ${
        ...
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div className="flex items-center gap-3">
            ...
            {!lockType ? (
              ...
              <span className="px-3 py-1 rounded-lg bg-indigo-600/15 text-indigo-400 font-bold text-xs flex items-center gap-1.5 border border-indigo-500/30">
          ...
          <div className="flex items-center gap-2">
            <button
              type="button"
              onClick={handleReset}
              className="px-3 py-2 rounded-xl text-xs font-semibold text-slate-400 hover:text-white hover:bg-slate-800 transition-colors flex items-center gap-1.5 cursor-pointer"
            >
            ...
            <button
              type="button"
              onClick={() => handleApply()}
              className="px-4 py-2 rounded-xl text-xs font-bold bg-indigo-600 hover:bg-indigo-500 text-white shadow-md shadow-indigo-600/25 transition-all cursor-pointer flex items-center gap-1.5"
            >
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 pt-2">
          ...
          <div
            className={`p-4 rounded-xl border flex flex-col justify-between ${
              theme === 'dark' ? 'bg-black/50 border-zinc-800' : 'bg-zinc-50 border-zinc-200 shadow-xs'
            }`}
          >
            <DualRangeSlider
            ...
            <div className="flex flex-wrap gap-1.5 pt-2">
              {[
                ...
                <button
                  ...
                  className={`text-[11px] px-2 py-0.5 rounded-lg font-semibold transition-colors cursor-pointer ${
          ...
          <div
            className={`p-4 rounded-2xl border flex flex-col justify-between ${
              theme === 'dark' ? 'bg-slate-950/60 border-slate-800' : 'bg-slate-50 border-slate-200 shadow-xs'
            }`}
          >
            <DualRangeSlider
                ...
                <button
                  ...
                  className={`px-3 py-1.5 rounded-xl text-xs font-semibold flex items-center gap-1.5 transition-all cursor-pointer ${
                    isSelected
                      ...
                      : 'bg-slate-100 text-slate-700 hover:bg-slate-200 hover:text-slate-950 border border-slate-200 shadow-xs'
                  }`}
                >
